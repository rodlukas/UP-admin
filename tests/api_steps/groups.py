...
def group_equal_to_context(group, context):
    ...
    return (
        ...
        and group["course"]["name"] == context.course.get("name")
        and set(memberships) == set(context.memberships)
        and group["active"] == context.active
    )
...
def group_dict(context):
    memberships = [
        {"client_id": helpers.find_client_with_full_name(context.api_client, membership).get("id")}
