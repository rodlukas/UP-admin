...
def attendancestate_equal_to_context(attendancestate, context):
    return (attendancestate['name'] == context.name and
            attendancestate['visible'] == context.visible)


def find_attendancestate_with_id(context, attendancestate_id):
    # nacti stav ucasti z endpointu podle id
    attendancestate_resp = context.api_client.get(f"{helpers.API_ATTENDANCESTATES}{attendancestate_id}/")
...
def step_impl(context, name):
    ...
    load_id_data_to_context(context, name)
    # najdi stav ucasti
    attendancestate_to_delete = helpers.find_attendancestate_with_name(context.api_client, context.name)
    ...
    context.resp = context.api_client.delete(f"{helpers.API_ATTENDANCESTATES}{attendancestate_to_delete['id']}/")
...
def step_impl(context, cur_name, new_name, new_visible):
    ...
    context.resp = context.api_client.put(f"{helpers.API_ATTENDANCESTATES}{attendancestate_to_update['id']}/",
...
@when(
    'user adds new attendance state "(?P<name>.*)" with visibility "(?P<visible>.*)"')
def step_impl(context, name, visible):
    ...
    save_old_attendancestates_cnt_to_context(context)
    # vlozeni stavu ucasti
    context.resp = context.api_client.post(helpers.API_ATTENDANCESTATES, attendancestate_dict(context))
